...
export function Header({ pathname }: { pathname: string }) {
  ...
  const handleBulldogClick = () => {
    ...
    const el = bulldogSectionRef.current;
    if (!el) return;
      ...
      <div className="flex items-center gap-4 min-w-0 flex-wrap">
        ...
        <nav className="flex items-center gap-3">
          ...
          {!isHome && (
            <a
              href="/"
              className="flex items-center gap-1.5 rounded-md border-2 border-zinc-400 px-2.5 py-1 text-sm font-semibold text-zinc-700 bg-white hover:bg-zinc-100 active:translate-y-0.5 active:shadow-[0_2px_0_rgb(161,161,170)] transition-all cursor-pointer shadow-[0_4px_0_rgb(161,161,170)] focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-1 outline-none"
            >
